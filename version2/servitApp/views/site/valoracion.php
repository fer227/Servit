...
<div class="container">
    ...
    <div class="center-align">
        <?= $form->field($model, 'recomendarias')->label(false)->radioList($opciones, ['item' =>function ($index, $label, $name, $checked, $value) {
            if($checked){
                return "<div class='row center-align'><label><input class='with-gap' type='radio' name='{$name}' value='{$value}' checked><span>{$label}</span></label></div>";
            }
            else{
                return "<div class='row center-align'><label><input class='with-gap' type='radio' name='{$name}' value='{$value}'><span>{$label}</span></label></div>";
            }
        }]) ?>
    </div>
