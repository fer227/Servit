...
class SiteController extends Controller
{
    ...
    public function actionInformes(){
        ...
        foreach ($zonas as $zona){
            ...
            foreach ($secciones as $seccion){
                $atiendes = Atiende::findAll(['id_zona' => $zona->id, 'numero' => $seccion->numero]);
                $repetiria = 0;
                $recomendaria = 0;
                $propinas = 0;
                $experiencia = 0;
                $ambiente = 0;
                $total = 0;
                foreach ($atiendes as $atiende){
                    $ganancias += $atiende->importe;
                    $propina += $atiende->propina;
                    $clientela += 1;
                    if($atiende->ambiente != null){
                        if($atiende->recomendaria == 2)
                            $recomendaria += 1;
                        elseif($atiende->recomendaria == 1)
                            $recomendaria += 0.5;
                        if($atiende->repetiria == 2)
                            $repetiria += 1;
                        elseif($atiende->repetiria == 1)
                            $repetiria += 0.5;
                        $propinas += $atiende->propina;
                        $experiencia += $atiende->experiencia;
                        $ambiente += $atiende->ambiente;
                        $total += 1;
                    }
                }
                $dato_tp['repetiria'] = round(($repetiria/$total)*100, 2);
                $dato_tp['recomendaria'] = round(($recomendaria/$total)*100, 2);
                $dato_tp['propinas'] = $propinas;
                $dato_tp['experiencia'] = round($experiencia/$total, 2);
                $dato_tp['ambiente'] = round($ambiente/$total,2);
                $dato_seccion[$seccion->numero] = $dato_tp;
            }
            $tmp = [];
            $tmp['nombre'] = $zona->nombre;
            $tmp['es_barra'] = $zona->es_barra;
            $tmp['datos'] = $dato_seccion;
            $dato_zona[$zona->nombre] = $tmp;
        }
        return $this->render('informe', ['datos' => $dato_zona, 'ganancias' => $ganancias, 'propinas' => $propina, 'clientela' => $clientela]);
    }
    ...
    public function actionPoliticaDatos()
    {
        return $this->render('politicadatos');
    }
    ...
    public function actionEditarEmpleado(){
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $usuarioApp->updateAttributes(['username' => $model->user, 'password' => $model->password, 'nombre' => $model->nombre, 'apellidos' => $model->apellido1 . " " . $model->apellido2]);
